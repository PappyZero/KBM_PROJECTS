...
import "@openzeppelin/contracts/token/ERC721/extensions/ERC721URIStorage.sol"; // Added this line

// Declaring the contract "NFT" as an ERC721 token and ownable.
contract NFT is ERC721, Ownable, ERC721URIStorage // Modified here
{
    using Strings for uint256;

    // Declaring the maximum supply of NFT collection as "10000".
    ...
    // Defining a constructor that runs when the contract is deployed.
    // The Constructor creates the ERC721 Token with a "Name" and a "Symbol"
    constructor() ERC721("PAPPYZERO PAPPY", "PPY") Ownable(msg.sender) // Pass the name and symbol here
    {
        ...
        // Setting the "total_supply" to equal the "tokens_reserved".
        total_supply = TOKENS_RESERVED;
    }

    // Creating a function that takes in a number of tokens that will be minted.
    function mint(uint256 _num_tokens) external payable {
        ...
        // less than the max number of tokens per transaction.
    ...
    // Declaring a function that takes in a number and sets the number to the price.
    ...
    // the smart contract and splits it into two wallets.
    ...
    function tokenURI(uint256 token_id) public view virtual override(ERC721, ERC721URIStorage) returns (string memory) {
        ...
        return bytes(current_base_uri).length > 0
            ? string(abi.encodePacked(current_base_uri, token_id.toString(), base_ext))
            : "";
    }
    ...
    function _burn(uint256 token_id) internal virtual override(ERC721, ERC721URIStorage) {
